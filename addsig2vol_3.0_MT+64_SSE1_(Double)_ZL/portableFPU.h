...
#ifdef USE_SSE
 uint32_t getSSEStateX86(void);
 void setSSEModDefault(uint32_t control);
 void modifySSEStateX86(uint32_t control, uint32_t mask);
#endif // USE_SSE

 void setRoundingMode(uint32_t round);                               ////Modified by ZL, accouding to the c99 ,Remove inline
 uint32_t _getFPUStateX86(void);
 void setFPUStateX86(uint32_t control);
 void setFPUModDefault(void);
 void assertFPUModDefault(void);
 void modifyFPUStateX86(const uint32_t control, const uint32_t mask);
 int32_t FastFtol(const float a);
